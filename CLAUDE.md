...
A fifth pseudo-source (`sites/backup-api.ts`, adapter name `API`) imports from the external "API Carros PT" aggregator (Railway) as a **backup**: its rows are marked `origin: "api"` and never override rows our own scraper produced — `upsertListing` skips API updates to scraper-origin rows (matching by `(source, externalId)` with URL fallback), the scraper reclaims API rows it re-finds, and `dedupeListings` hides the API copy of a same-source pair. `BACKUP_API_KEY` is only needed for on-demand detail.

Image URLs from scraped listings are stored as JSON arrays (hotlink to origin CDNs, never downloaded). Listing details (description, equipment, color, etc.) are fetched on demand when someone opens the listing for the first time (`detailsFetchedAt` is set); if the origin site blocks the fetch (e.g. PiscaPisca's Cloudflare), `ensureListingDetail` falls back to the backup API's `POST /listings/detail`.
